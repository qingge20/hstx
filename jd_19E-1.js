...
!(async() => {
    ...
    for (let i = 0; i < cookiesArr.length && true; i++) {
     ...
     let lnTotalAcc = Math.ceil(cookiesArr.length / 2);
...
async function promote_sign() {
    await gettrl();
    let body = { "ss": { "extraData": { "log":$.ttLog, "sceneid": "RAhomePageh5" }, "secretp": secretp, "random": $.ttrandomNum } };
    return new Promise((resolve) => {
...
async function promote_collectScore(taskToken, taskId) {
    await gettrl();
    let body = { "taskId": taskId, "taskToken": taskToken, "actionType": 1, "ss": { "extraData": { "log": $.ttLog, "sceneid": "RAhomePageh5" }, "secretp": secretp, "random": $.ttrandomNum } };
...
async function gettrl(){
    await $.wait(parseInt(Math.random() * 3500 + 600, 10))
    return new Promise(async resolve => {
        ttlogs = await trl();
        if (ttlogs && ttlogs.code == 0) {
            $.ttrandomNum = String(Object.keys(ttlogs.data)[0]);
            $.ttLog = String(Object.values(ttlogs.data)[0]);
        } else {
...
function trl() {
    return new Promise(async resolve => {
        $.get({url: `http://hz.feverrun.top:99/share/get/ttlogs`, timeout: 20000}, (err, resp, data) => {
            try {
                // console.log(`koilogs:${data}`);
                if (err) {
                    // console.log(`${JSON.stringify(err)}`);
